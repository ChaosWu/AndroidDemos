 ...
 * 
 */
public class DetectTouchFreeDrawBitmap extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.ui_graphics_touch_free_draw);

		btnLoadImage = (Button) findViewById(R.id.free_draw_loadimage);
		textSource = (TextView) findViewById(R.id.free_draw_sourceuri);
		imageResult = (ImageView) findViewById(R.id.free_draw_result);
		imageDrawingPane = (ImageView) findViewById(R.id.free_draw_drawingpane);
		...
		imageResult.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				 ...
				 * If auto-generated 'false', your code can detect ACTION_DOWN
				 * only, cannot detect ACTION_MOVE and ACTION_UP.
				 */
				return true;
			}
		});
	...
	private void drawOnRectProjectedBitMap(ImageView iv, Bitmap bm, int x, int y) {
		if (x < 0 || y < 0 || x > iv.getWidth() || y > iv.getHeight()) {
			...
			canvasDrawingPane.drawRect(startPt.x, startPt.y, projectedX,
					projectedY, paint);
			imageDrawingPane.invalidate();

			textSource.setText(x + ":" + y + "/" + iv.getWidth() + " : "
		...
		if (resultCode == RESULT_OK) {
			...
			case RQS_IMAGE1:
				...
				try {
					...
					bitmapMaster = Bitmap.createBitmap(tempBitmap.getWidth(),
